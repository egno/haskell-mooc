...
instance (Ord a) => Semigroup (Set a) where
  (<>) = union
   where
    union (Set []) b       = b
    union (Set (a : as)) b = Set as `union` add a b
...
passwordAllowed s (ContainsSome c) = any (`elem` s) c
passwordAllowed s (DoesNotContain c) = not $ passwordAllowed s (ContainsSome c)
