...
allValues condition (Node n left right) = condition n && allValues condition left && allValues condition right
...
isOrdered (Node n left right) =
  ...
  where
    ...
    isOrdered' f val (Node n _ _) = f n val
...
walk (StepL:xs) (Node n left _)  = walk xs left
walk (StepR:xs) (Node n _ right) = walk xs right
...
set [] val (Node n left right)         = Node val left right
set (StepL:xs) val (Node n left right) = Node n (set xs val left) right
set (StepR:xs) val (Node n left right) = Node n left (set xs val right)
...
search val tree = search' val tree []
  where
    ...
    search' val (Node n left right) path
      | val == n = Just path
      | otherwise = case searchPath of
        (Just leftPath, _)  -> Just leftPath
        (_, Just rightPath) -> Just rightPath
        _                   -> Nothing
      where
        searchPath = (search' val left (path ++ [StepL]), search' val right (path ++ [StepR]))
