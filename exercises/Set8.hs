...
rectangle x0 y0 w h = Shape f
 where
  f (Coord x y) = x >= x0 && x0 + w > x && y >= y0 && y0 + h > y
...
paintSolid color (Shape f1) (Picture p1) = Picture f
 where
  f coord = if f1 coord then color else p1 coord
...
paint (Picture p1) (Shape f1) (Picture p2) = Picture f
 where
  f coord = if f1 coord then p1 coord else p2 coord
...
instance Transform Blur where
  apply Blur (Picture f) = Picture blur 
    where 
    blur (Coord x y) = Color (avg getRed) (avg getGreen) (avg getBlue)
      where
      avg fc = div (sum $ map fc neighbours) (length neighbours)
      neighbours = [f (Coord (x+dx) (y+dy)) | dx <- [-1,0, 1], dy <- [-1,0,1], dx == 0 || dy == 0]
