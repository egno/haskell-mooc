...
firstHalf s = take cnt s
    where cnt = div (length s+1) 2
...
whileRight check x = check' $ check x
    where
        check' (Left b)  = b
        check' (Right a) = check' $ check a
...
joinToLength n s = filter ((==n).length) [a ++ b | a <-s , b <- s]
...
sumRights s =  sum $ map value $ filter isRight s
    where
        value (Right x) = x
