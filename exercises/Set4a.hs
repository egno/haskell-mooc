...
longest :: (Ord a) => [[a]] -> [a]
longest = minimumBy (comparing (\l -> (-length l, head l)))
...
incrementKey k = map (\(k',v) -> (k', if k'==k then v+1 else v))
...
freqs = foldr (Map.alter counter) Map.empty
    where
        counter :: Maybe Int -> Maybe Int
        counter (Just n) = Just (n+1)
        counter _        = Just 1
...
transfer from to amount bank
    ...
    | otherwise = bank'
    where
        Just from' = Map.lookup from bank
        Just to' = Map.lookup to bank
        bank' = Map.insert to (to'+amount) $ Map.insert from (from'-amount) bank
...
maxIndex arr = fst $ foldr (\l rs -> if snd l > snd rs then l else rs) x xs
    where (x:xs) = assocs arr
