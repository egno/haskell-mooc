...
danger cand = any isDanger
  where
    isDanger queen =
      any (\f -> f cand queen) [sameRow, sameCol, sameDiag, sameAntidiag]
...
fixFirst _ [] = Nothing
fixFirst n (s:sx) = checkDanger s sx
  where
    checkDanger (r, c) sx
      | c > n            = Nothing
      | danger (r, c) sx = checkDanger (r, c + 1) sx
...
finish n (s:sx)
  | length sx < n = finish n (step n (s : sx))
  | otherwise     = sx
